...
async function browsePath(path: string) {
  ...
  try {
    ...
    console.log('[HermesFilesPage] Loaded', entries.value.length, 'entries for path:', path, 'workspace:', currentWorkspace.value)
  } catch (e: any) {
    console.error('[HermesFilesPage] browsePath error:', e)
...
async function openPreview(file: FileEntry) {
  ...
  try {
    ...
    console.error('[HermesFilesPage] openPreview error:', e)
...
async function openEditor(file: FileEntry) {
  ...
  try {
    ...
    console.error('[HermesFilesPage] openEditor error:', e)
    message.error(t('pages.files.readFileFailed') + ': ' + (e?.message || ''))
  } finally {
    fileLoading.value = false
  }
...
async function saveFile() {
  ...
  fileLoading.value = true
  try {
    console.log('[HermesFilesPage] Saving file:', selectedFile.value.path)
    ...
    console.error('[HermesFilesPage] saveFile error:', e)
...
async function createEntry() {
  ...
  try {
    ...
    console.log('[HermesFilesPage] Creating:', createType.value, fullPath)
    ...
    console.error('[HermesFilesPage] createEntry error:', e)
...
async function deleteEntry(file: FileEntry) {
  try {
    console.log('[HermesFilesPage] Deleting:', file.path)
    ...
    console.error('[HermesFilesPage] deleteEntry error:', e)
...
async function renameEntry() {
  ...
  try {
    ...
    console.log('[HermesFilesPage] Renaming:', oldPath, '->', newPath)
    ...
    console.error('[HermesFilesPage] renameEntry error:', e)
    message.error(t('pages.files.createFailed') + ': ' + (e?.message || ''))
  } finally {
...
async function handleUpload({ file, onFinish, onError }: { file: UploadFileInfo, onFinish: () => void, onError: (error: Error) => void }) {
  ...
  try {
    ...
    console.log('[HermesFilesPage] Uploading:', fullPath, 'size:', file.file.size)
    ...
    console.error('[HermesFilesPage] handleUpload error:', e)
...
async function uploadPastedImage(file: File) {
  ...
  try {
    ...
    console.error('[HermesFilesPage] uploadPastedImage error:', e)
    message.error(t('pages.files.uploadFailed') + ': ' + (e?.message || ''))
  } finally {
  ...
  <NSpace vertical :size="16">
    <NCard :title="t('routes.files')" class="app-card">
      <template #header-extra>
        <NSpace :size="8">
          <NButton size="small" class="app-toolbar-btn app-toolbar-btn--refresh" :loading="loading" @click="browsePath(currentPath)">
